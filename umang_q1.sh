...
cd umang
id=$(aws autoscaling describe-auto-scaling-groups --auto-scaling-group-names "shellscripting" --querry 'AutoScalingGroups[0].Instances[0].InstanceId' --output text)
ip=$(aws ec2 describe-instances --instance-ids $id --querry 'Reservations[0].Instances[0].PublicIpAddress' --output text)
